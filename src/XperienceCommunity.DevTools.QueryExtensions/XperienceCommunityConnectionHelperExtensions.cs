...
namespace CMS.DataEngine
{
    ...
    public static class XperienceCommunityConnectionHelper
    {
        ...
        public static async Task<DataSet> ToDataSetAsync(this Task<IDataReader> reader)
        {
            if (reader is null)
            {
            ...
            var readerVal = await reader;
            do
            {
                var table = new DataTable();
                table.Load(readerVal);
                ds.Tables.Add(table);
            } while (!readerVal.IsClosed);
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        public static async Task<DataSet> ToDataSetAsync(this Task<DbDataReader> reader)
        {
            if (reader is null)
            {
                return new DataSet().AddEmptyTable();
        ...
        {
            var query = await GetCachedQueryAsync(queryClassName, queryCodeName, token: token) ?? throw new Exception($"No query found for class name [{queryClassName}] and query name [{queryCodeName}]");
